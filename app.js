...
let sessionData = {
    ...
    muhatap: 1, // 1: Yapay Zeka, 2: İnsan
...
function initAppNavigation() {
    ...
    btnD1InitNext.addEventListener("click", () => {
        ...
        // 1: Evet (İsrafı önle -> Karşıtı: wstfl), 2: Hayır (Eşitliği koru -> Karşıtı: unfrns)
        const chatLink1 = document.getElementById("btn-d1-chat-link");
        if (sessionData.d1_initial_choice === 1 || choiceVal === "1" || choiceVal === "Evet") {
            sessionData.d1_assigned_model = "wstfl";
            if (chatLink1) chatLink1.href = "https://mocolabtester.github.io/wstfl/?d=1";
        } else {
            sessionData.d1_assigned_model = "unfrns";
            if (chatLink1) chatLink1.href = "https://mocolabtester.github.io/unfrns/?d=1";
        }
    ...
    if (linkD1Chat && btnD1AiDone) {
        linkD1Chat.addEventListener("click", (e) => {
            ...
            btnD1AiDone.disabled = false;
            saveSurveySession();
            openChatModal(linkD1Chat.href, "Yapay Zeka Asistanı (1. Görüşme)");
        });
    }
    ...
    btnD2InitNext.addEventListener("click", () => {
        ...
        // DESTEKLEYİCİ AI (Katılımcının İkilem 2'deki kararına kesinlikle DESTEKLEYİCİ model atanır):
        // 1: Evet (İsrafı önle -> Destekleyen: unfrns), 2: Hayır (Eşitliği koru -> Destekleyen: wstfl)
        const chatLink2 = document.getElementById("btn-d2-chat-link");
        if (sessionData.d2_initial_choice === 1 || choiceVal === "1" || choiceVal === "Evet") {
            sessionData.d2_assigned_model = "unfrns";
            if (chatLink2) chatLink2.href = "https://mocolabtester.github.io/unfrns/?d=2";
        } else {
            sessionData.d2_assigned_model = "wstfl";
            if (chatLink2) chatLink2.href = "https://mocolabtester.github.io/wstfl/?d=2";
        }
    ...
    if (linkD2Chat && btnD2AiDone) {
        linkD2Chat.addEventListener("click", (e) => {
            ...
            btnD2AiDone.disabled = false;
            saveSurveySession();
            openChatModal(linkD2Chat.href, "Yapay Zeka Asistanı (2. Görüşme)");
        });
    }
